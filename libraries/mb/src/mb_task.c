...
void MBi_InitTaskThread (void * p_work, u32 size)
{
	...
	if (!mbi_task_work) {
		...
		OS_CreateThread(p->th, MBi_TaskThread, p, (u8 *)(p + 1) + size, size, OS_THREAD_PRIORITY_MIN);
...
void MBi_SetTask (MBiTaskInfo * pt, MB_TASK_FUNC task, MB_TASK_FUNC callback, u32 priority)
{
	...
	if (priority > OS_THREAD_PRIORITY_MAX) {
		...
		if (priority == MB_TASK_PRIORITY_ABOVE) {
			priority = (u32)((cur_priority > OS_THREAD_PRIORITY_MIN) ? (cur_priority - 1) : OS_THREAD_PRIORITY_MIN);
		} else if (priority == MB_TASK_PRIORITY_BELOW)   {
			priority = (u32)((cur_priority < OS_THREAD_PRIORITY_MAX) ? (cur_priority + 1) : OS_THREAD_PRIORITY_MAX);
