...
BOOL MB_ReadSegment (FSFile * file, void * buf, u32 len)
{
	...
	if (len >= ROM_HEADER_SIZE_FULL + 4) {
		...
		if (file) {
			...
			FS_InitFile(own_bin);
			(void)FS_OpenFileDirect(own_bin, FS_FindArchive("rom", 3), 0, (u32)(own_size + MB_AUTHCODE_SIZE), (u32) ~0);
			file = own_bin;
			bak_pos = FS_GetPosition(file);
...
BOOL MB_RegisterFile (const MBGameRegistry * game_reg, const void * buf)
{
	...
	pPwork->fileinfo[fileID].cache_list = (MBiCacheList *)((u32)buf + ROM_HEADER_SIZE_FULL + MB_AUTHCODE_SIZE);
	pPwork->fileinfo[fileID].card_mapping = (u32 *) ((u32)buf + ROM_HEADER_SIZE_FULL + MB_AUTHCODE_SIZE + sizeof(MBiCacheList));
