...
SDK_COMPILER_ASSERT(32 + sizeof(MBiParam) + 32 + sizeof(MB_CommPWork) + 32 + WM_SYSTEM_BUF_SIZE <= MB_SYSTEM_BUF_SIZE);
SDK_COMPILER_ASSERT(32 + sizeof(MBiParam) + 32 + sizeof(MB_CommCWork) + 32 + WM_SYSTEM_BUF_SIZE <= MB_CHILD_SYSTEM_BUF_SIZE);
...
static void MBi_OnInitializeDone (void)
{
    ...
    errcode = WM_SetLifeTime(MBi_ParentCallback, mbi_life_table_no, mbi_life_cam, mbi_life_frame, mbi_life_mp);
...
static void MBi_ChildCallback (void * arg)
{
    ...
    case WM_APIID_INITIALIZE:
    {
        ...
        errcode = WM_SetLifeTime(MBi_ChildCallback, mbi_life_table_no, mbi_life_cam, mbi_life_frame, mbi_life_mp);
        ...
        case WM_STATECODE_PARENT_FOUND:
        {
            ...
            for (i = 0; i < p_param->found_parent_count; ++i) {
                if (WM_IsBssidEqual(buf->macAddress, p[i].scan_data.macAddress)) {
                    ...
                    MI_DmaCopy16(WM_DMA_NO, &p_param->parentInfoBuf, p_param->parent_info[i].parentInfo, WM_BSS_DESC_SIZE);
...
int MB_Init (void * work, const MBUserInfo * user, u32 ggid, u32 tgid, u32 dma)
{
    ...
    if (mbc && mbc->isMbInitialized) {
        ...
        MB_CommCommonWork * const p_com = (MB_CommCommonWork *) ((((u32)p_parm) + sizeof(*p_parm) + 0x1F) & ~0x1F);
        OSIntrMode enabled;
...

        {
...

            {
                ...
                p_parent->beaconPeriod = (u16)(MB_BEACON_PERIOD_DEFAULT + MBi_GetBeaconPeriodDispersion());
...
static int MBi_StartParentCore (int channel)
{
    ...
    MI_CpuClear16((void *)((u32)pPwork + sizeof(MB_CommCommonWork)), sizeof(MB_CommPWork) - sizeof(MB_CommCommonWork));
...
int MB_StartChild (void)
{
    ...
    MI_CpuClear16((void *)((u32)pCwork + sizeof(MB_CommCommonWork)), sizeof(MB_CommCWork) - sizeof(MB_CommCommonWork));
...
#pragma unused(tmptt)
    ...
    int errcode;
    errcode = WM_SetMPDataToPort(callback, sendData, sendDataSize, pollbmp, WM_PORT_BT, WM_PRIORITY_LOW);
