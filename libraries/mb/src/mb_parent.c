...
static void MBi_CommParentRecvDataPerChild (void * arg, u16 child)
{
	...
	case MB_COMM_TYPE_CHILD_CONTINUE:
		if (state == MB_COMM_PSTATE_SEND_PROCEED) {
			...
			pPwork->fileinfo[fileid].nextb = MBi_calc_nextsendblock(pPwork->fileinfo[fileid].nextb, hd.data.req);
...
static int MBi_CommParentSendDLFileInfo (void)
{
	...
	return MBi_BlockHeaderEnd(sizeof(MBDownloadFileInfo) + MB_COMM_PARENT_HEADER_SIZE, pollbmp, pPwork->common.sendbuf);
}
...
static void MBi_ReloadCache (MBiTaskInfo * p_task)
{
	...
	if (FS_OpenFileDirect(file, arc, p_info->src, p_info->src + p_info->len, (u32) ~0)) {
