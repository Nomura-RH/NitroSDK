...
typedef struct WFSFATFormat {
	u32 top;
	u32 bottom;
} PLATFORM_STRUCT_PADDING_FOOTER WFSFATFormat;
...
typedef struct WFSOVLFormat {
	...
	u32 compressed :24;
#endif
} PLATFORM_STRUCT_PADDING_FOOTER WFSOVLFormat;
...
} PLATFORM_STRUCT_PADDING_FOOTER WFSTableFormat;
...
typedef struct WFSMessageFormat {
	...
	u8 packet_lo;
	u8 reserved[3];
} PLATFORM_STRUCT_PADDING_FOOTER WFSMessageFormat;
...
BOOL WFS_LoadTable(WFSTableFormat * table, MIAllocator * allocator, MIDevice * device, u32 fatbase, u32 overlay);
void WFS_ParseTable(WFSTableFormat * table);

PLATFORM_ATTRIBUTE_INLINE BOOL WFS_SendMessage (WBTContext * context, WBTEventCallback callback, int bitmap,
...
PLATFORM_ATTRIBUTE_INLINE BOOL WFS_SendMessageLOCK_REQ (WBTContext * context, WBTEventCallback callback, int bitmap, u32 offset, u32 length)
{
	return WFS_SendMessage(context, callback, bitmap, WFS_MSG_LOCK_REQ, offset, length);
}

PLATFORM_ATTRIBUTE_INLINE BOOL WFS_SendMessageLOCK_ACK (WBTContext * context, WBTEventCallback callback, int bitmap, u32 id)
{
	return WFS_SendMessage(context, callback, bitmap, WFS_MSG_LOCK_ACK, id, (u32)(id != 0));
}

PLATFORM_ATTRIBUTE_INLINE BOOL WFS_SendMessageUNLOCK_REQ (WBTContext * context, WBTEventCallback callback, int bitmap, u32 id)
{
	return WFS_SendMessage(context, callback, bitmap, WFS_MSG_UNLOCK_REQ, id, TRUE);
}

PLATFORM_ATTRIBUTE_INLINE BOOL WFS_SendMessageUNLOCK_ACK (WBTContext * context, WBTEventCallback callback, int bitmap, u32 id)
{
